...
public class Ex14_11 {
    public static void main(String[] args) {
        ...
        Map<Student3.Level, List<Student3>> stuByLevel1 = Stream.of(stuArr).collect(groupingBy(s -> {
        ...
        for (Student3.Level key : keySet) {
            ...
            for (Student3 student3 : stuByLevel1.get(key)) {
                System.out.println(student3);
            }
            System.out.println();
        }
        ...
        Map<Student3.Level, Long> stuCntByLevel1 = Stream.of(stuArr).collect(groupingBy(s -> {
            if (s.getScore() >= 200) return Student3.Level.HIGH;
            else if (s.getScore() >= 100) return Student3.Level.MID;
            else return Student3.Level.LOW;
        }, counting()));

        for (Student3.Level key : stuCntByLevel1.keySet()) {
            System.out.printf("[%s] - %d명, ", key, stuCntByLevel1.get(key));

        }
        System.out.println();

        System.out.println("4. 다중그룹화(학년별, 반별)");
        Map<Integer, Map<Integer, List<Student3>>> stuByHakAndBan = Stream.of(stuArr).collect(groupingBy(Student3::getHak, groupingBy(Student3::getBan)));
        ...
        Map<Integer, Map<Integer, Student3>> topStuByHakAndBan = Stream.of(stuArr).collect(groupingBy(Student3::getHak, groupingBy(Student3::getBan, collectingAndThen(maxBy(comparingInt(Student3::getScore)), Optional::get))));
        ...
        Set<String> keySet1 = stuByScoreGroup.keySet(); //중복 제거
